    ...
    public void handleAttribute(AttributeParsingEvent event) {
        if (event.getName().endsWith(".edition.xml")) {
            ...
            try {
                ...
                if (doc == null) {
                    addFailure(event.getName(), "Could not parse xml from " + event.getName());
                    return;
                }
            } catch (IOException e) {
                throw new RuntimeException(e);
            }
            final String xpathForEditionNumberXPath = "mods:mods/mods:relatedItem[@type=\"host\"]/mods:part/mods:detail@type=\"edition\"]/mods:number";
            String xpathForEditionNumber = xpath.selectString(doc, xpathForEditionNumberXPath);
            String editionIDFromNode = event.getName().split("/")[4];
            String nodeEditionNumber = editionIDFromNode;
        }
